...
| Phase 1 — AOT/interpreter parity | Dynamic dispatch + method tables | ⏳ PLANNED | Classes/inheritance/`super`, `import`, `try`/`except`/`finally`, `raise`, `yield` are interpreter-only. Land each in AOT in this order: exceptions → modules/imports → classes → generators. **classes (definitions, instantiation, inheritance, `super()`) are now landed in AOT codegen (static-dispatch model)**; generators (`yield` statements and generator expressions) are now landed too. Record each in `docs/adr/` and update `docs/language.md`. |
...
| Phase 3 — memory model & optimization | Document the memory model | ✅ DONE (ADR 0135) | `docs/adr/0135-memory-model.md`: AOT fixed heap + free-list + rebind-free + escape-analysis elision (ADR 0134); interpreter generational GC (ADR 0132); stress harness (ADR 0133); safe GC collection points.
