...
| Phase 1 — AOT/interpreter parity | Runtime heap + GC in AOT (milestone 1: runtime heap + boxed mutable lists with `list.append()` on variables and `print(list)` — done, ADR 0009; milestone 2: `len(x)` on runtime list variables reads the mutated length via the heap — done; full mark-and-sweep GC and runtime dict/set objects are follow-on) | ⏳ PLANNED | Today lists/dicts/sets/strings are compile-time globals only. Introduce an object heap, boxed values, and a refcount/GC pass so AOT programs can mutate runtime collections. Mirror the interpreter's `Collect()` contract. |
