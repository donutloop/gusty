...
| Phase 1 — AOT/interpreter parity | Dynamic dispatch + method tables | ⏳ PLANNED | Classes/inheritance/`super`, `import`, `try`/`except`/`finally`, `raise`, `yield` are interpreter-only. Land each in AOT in this order: exceptions → modules/imports → classes → generators. **classes (definitions, instantiation, inheritance, `super()`) are now landed in AOT codegen (static-dispatch model)**; generators remain. Record each in `docs/adr/` and update `docs/language.md`. |
